...
SealMicServer: 应用服务器，负责各种业务数据存储，比如用户、房间、APP版本
...
DemoServer 服务无状态，支持水平扩展，通过前置负载均衡实现分布式部署，提升系统吞吐
...
|组件|版本|说明|
|---|---|---|
|Mysql|5.6+|业务数据，存储用户、房间、APP 版本等信息|
|Redis|4.0+|缓存数据库，缓存用户、房间、排麦用户、麦位等信息|
|Jdk|1.8|Java 版本|
...
#### 接口请求
部分接口需要校验登录状态，需要在请求 Header 中携带 Authorization 字段
