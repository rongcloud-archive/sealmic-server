...
public class RedisTemplateFactory {
    ...
    @Bean(name = "micPositionRedisTemplate")
    public RedisTemplate<String, MicPositionInfo> micPositionRedisTemplate(
        RedisSerializer<String> keyStringSerializer) {
        RedisTemplate<String, MicPositionInfo> template = new RedisTemplate<>();
        template.setConnectionFactory(connectionFactory);
        template.setKeySerializer(keyStringSerializer);
        template.setHashValueSerializer(new GenericJackson2JsonRedisSerializer());
        return template;
